...
class TestUpdateUserLocation(APITestCase):
    ...
    def good_test(self):
        user = MagicMock()
        addr = MagicMock()
        views.update_user_location(user, addr)
...
class TestGetResponseFromAudio(APITestCase):
    ...
    def good_test(self, mock_recognizer, mock_base64, mock_open, mock_audiofile, mock_record, mock_recognize_sphinx,
                  mock_ai_response):
    ...
    @patch("recognizer.recognize_sphinx")
    @patch("get_ai_response")
    def bad_test(self, mock_recognizer, mock_base64, mock_open, mock_audiofile, mock_record, mock_recognize_sphinx,
                 mock_ai_response):
